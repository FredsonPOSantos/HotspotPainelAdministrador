...
// [VERSÃO 13.5 - Revisão Final da Sintaxe em loadPermissionsMatrix]
...
if (window.initSettingsPage) {
    ...
    window.initSettingsPage = () => {
        ...
        console.log("A inicializar a página de Configurações (V13.5)...");
        ...
        const loadPermissionsMatrix = async () => {
            ...
            try {
                ...
                if(theadRow) {
                     ...
                     permissionsData.roles.forEach(roleKey => { theadRow.innerHTML += `<th>${roleKey.charAt(0).toUpperCase() + roleKey.slice(1)}</th>`; roleCount++; });
                } else { roleCount = 4; console.error("Thead permissions table not found!"); }
                const totalColumns = roleCount + 1;
